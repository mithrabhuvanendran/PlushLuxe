...
const FilterSidebar = () => {
  ...
  const handleFilterChange = (e) => {
    ...
    // console.info({name, value, checked, type});
    const newFilters = {...filters}

    if(type === "checkbox") {
      if(checked) {
        newFilters[name] = [...(newFilters[name] || []), value] // append values to the array i.e. newFilters[size] when checkbox is clicked
      } else {
  ...
  const updateURLParams = (newFilters) => {
    ...
    Object.keys(newFilters).forEach((key) => {
      if(Array.isArray(newFilters[key] && newFilters[key].length > 0 )) {
        params.append(key, newFilters[key].join(",")) // (key, newFilters[key] means the value of that key)
  ...
  return (
      ...
      <div className="p-4">
        ...
        <button onClick={() => setIsSidebarOpen(!isSidebarOpen)} className="text-xl text-center mb-4 lg:hidden">
        <IoMdClose />
        </button>
        </div>
        {/* Category filter */}
